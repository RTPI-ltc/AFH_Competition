...
def call_llm_via_sdk(system: str, user: str, json_mode: bool = False) -> str:
    """同步桥接：通过 SDK 层调用 LLM，供现有同步节点渐进迁移使用。"""
    import asyncio
    ...
    return asyncio.run(client.call(system, user, stream=False, json_mode=json_mode))
