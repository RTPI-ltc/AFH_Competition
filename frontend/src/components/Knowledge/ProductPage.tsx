...
const initialForm = {
  sku_id: '', product_name: '', brand: '云璟珠宝', category_l1: '黄金', category_l2: '足金',
...
export function ProductPage({ onBack }: ProductPageProps) {
  ...
  const handleAdd = async () => {
    ...
    try {
      await addProduct({
        ...(form.sku_id.trim() ? { sku_id: form.sku_id.trim() } : {}),
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.sku_id} onChange={e => setForm({...form, sku_id: e.target.value})}
                    placeholder="留空自动生成" className="w-full px-2 py-1.5 text-xs border rounded-lg focus:ring-1 focus:ring-indigo-500" />
                </div>
                <div>
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button onClick={handleAdd} disabled={adding || !form.product_name}
